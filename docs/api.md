...
listener and the sink are unchanged.
...
**listen** (intended). `schema.sql` now installs a trigger that runs
...
To enable push on a database that was set up before this change, re-apply
...
Caddy proxies WebSockets and SSE without extra configuration.
